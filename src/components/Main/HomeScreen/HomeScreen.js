  ...
  useEffect(() => {
    const fetchData = () => {
      if (token !== null && isFocused) {
        ...
        fetchWithAu(API_URL + "user/get-favorite-courses", "GET", token).then(
          (data) => {
        ...
        fetchWithoutAu(
          API_URL + "course/courses-user-favorite-categories",
          "POST",
          { userId: info.id }
        ).then(
          (data) => {
        ...
        fetchWithAu(API_URL + "user/get-process-courses", "GET", token).then(
          (data) => {
            setLoading(false);
    ...
    <>
      <View
        style={{
          flex: 1,
          paddingBottom: 20,
          justifyContent: token === null ? "center" : "flex-start",
        }}
      >
        {token === null ? (
          <Text style={[styles.text, { color: theme.TEXT_COLOR }]}>
            {languageConstant.REQUIRE_LOGIN}
          </Text>
        ) : (
          <ScrollView showsVerticalScrollIndicator={false}>
            <CourseSection
              ...
              style={{ marginTop: 30 }}
              title={languageConstant.FAVORITE_COURSES}
            />
            <CourseSection
              courses={coursesByCategories}
              navigation={navigation}
              style={{ marginTop: 40 }}
              title={languageConstant.COURSE_IN_CATEGORY}
            />
            <RegisterSection
              courses={coursesRegistered}
              navigation={navigation}
              style={{ marginTop: 40 }}
              title={languageConstant.COURSE_LEARNING}
            />
          </ScrollView>
        )}
      </View>
