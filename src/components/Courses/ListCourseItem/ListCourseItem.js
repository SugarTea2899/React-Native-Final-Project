...
const ListCourseItem = ({ course, navigation }) => {
  ...
  const renderDate = () => {
    if (course.updatedAt) {
      return course.totalHours
        ? `${new Date(
            course.updatedAt
          ).toLocaleDateString()}  -  ${course.totalHours.toFixed(2)} hours`
  ...
  return (
    <TouchableOpacity
      onPress={() =>
        navigation.navigate(languageConstant.COURSE_DETAIL, { course: course })
      }
    >
      <View style={styles.container}>
        ...
        <View style={styles.textGroup}>
          ...
          <Text style={[styles.otherText, { color: theme.TEXT_COLOR_BLUR }]}>
            {course.instructorName ? course.instructorName : course.name}
          </Text>
          <Text style={[styles.otherText, { color: theme.TEXT_COLOR_BLUR }]}>
            {renderDate()}
          </Text>
